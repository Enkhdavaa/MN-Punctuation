...
val_data = val_data[:int(len(val_data)*(data_factor*2))] # limit data to x%
...
with open("hyperparamert-bert.txt", "w") as f:
    print(result,file=f)
    f.close()

# ---- result 50 runs ----
#BestRun(run_id='44', objective=0.9041162060518235, hyperparameters={'learning_rate': 0.0001288100542701203, 'num_train_epochs': 4, 'per_device_train_batch_size': 4, 'weight_decay': 0.029524700063923954, 'adam_epsilon': 3.020858051874141e-07, 'gradient_accumulation_steps': 1})

# ---- ressult 200 runs ---
# BestRun(run_id='198', objective=0.9330957734669766, hyperparameters={'learning_rate': 8.304543067256128e-05, 'num_train_epochs': 2, 'per_device_train_batch_size': 8, 'weight_decay': 0.03598207778585443, 'adam_epsilon': 2.0572584966601974e-09, 'gradient_accumulation_steps': 2})
